...
def index_view(request):
    return render(request,'hospital/main_index.html')
...
def find_nearest_hospital(request):
    ...
    if request.method == "POST":
        ...
        nearest_hospital = find_nearest_hospitals(user_lat, user_lng)
        if nearest_hospital:
            return redirect("hospital", hospital_name=nearest_hospital['name'])
        else:
            return "No hospitals found nearby."
    else:
        return "No hospital found"

def hospital(request, hospital_name):
    hospital = next((h for h in hospitals if h['name'] == hospital_name), None)
    if hospital:
        return render(request,'hospital/hospital1.html', {"hospital" : hospital})
    else:
        return "Hospital not found."


def find_nearest_hospitals(user_lat, user_lng):
